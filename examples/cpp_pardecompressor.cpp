...
void decompress_points(std::string fname, las_file_t *las_file, const las_vlr *laszip_vlr, bool parallel)
{    
    lazrs::LasZipDecompressor decompressor(
        fname, laszip_vlr->data, laszip_vlr->record_len, las_file->header.offset_to_point_data, parallel);
